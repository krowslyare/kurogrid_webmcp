...
Appointment updates always return the customer to a private status link. With
no email provider configured, the demo shows an honest in-product email
preview. To send the same update through Resend, set these server-only values:
...
idempotency key per appointment status and time; a provider failure does not
roll back the appointment or invalidate its private status link.
