...
export function CopyAgentPrompt({ prompt }: CopyAgentPromptProps) {
  ...
  async function copyPrompt(nextDestination = "your AI agent") {
    ...
    try {
      ...
      setErrorMessage("We couldn't copy that. Try again.");
      setStatus("error");
    }
  ...
  return (
    <div className="clinic-copy-action">
      <div className="clinic-agent-request">
        <label htmlFor={promptId}>Your request</label>
        <textarea
          id={promptId}
          onChange={(event) => {
      ...
      <p id={feedbackId} aria-live="polite" className={`clinic-copy-feedback is-${status}`}>
        {status === "copied"
          ...
          : status === "error"
            ? errorMessage
            : "We include this page so current appointment times stay with your request."}
      </p>
    </div>
  );
}
